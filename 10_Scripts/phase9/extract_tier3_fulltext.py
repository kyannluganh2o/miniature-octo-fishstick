#!/usr/bin/env python3
"""Create the read-only-derived machine-readable layer for eligible Tier 3 PDFs.

The script accepts only locked Tier 3 records whose inventory entry says that
...
def process(paper_id: str, master: dict[str, dict[str, str]], inventory: dict[str, dict[str, str]], tiers: dict[str, dict[str, str]]) -> dict[str, object]:
    ...
    if tier["reading_tier"] != "tier3" or tier["reading_status"] != "not_started":
        raise ValueError(f"{paper_id}: not an unprocessed Tier 3 paper")
    ...
    with tempfile.TemporaryDirectory(prefix=f"phase9_{paper_id}_") as temp:
        ...
        log = {
            ...
            "scientific_reading_mode": "claim_targeted",
            "reading_tier": "tier3",
