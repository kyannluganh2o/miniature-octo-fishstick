...
TABLES = OUT / "tables"
RENDERED = ROOT / "tmp" / "pdfs" / "phase14r_final_pages"
...
def render_page(pdf: Path, page: int, dpi: int = 260) -> Path:
...
PANEL_SPECS = [
    PanelSpec("SP02-01", "B009", "02_PDF_Raw/B/B-H2-04_2014_Hamzehloo_LES_Underexpanded_Hydrogen_MethaneJets.pdf", 11, "Fig. 8", "sequence", (0.04, 0.04, 0.96, 0.96), "Axial-Mach evolution and Mach-disk formation", "Hydrogen; NPR 10; transient LES", "S06", "FIG-02", "Page crop retaining time labels and contour legend", "LOC-B009-0005"),
    PanelSpec("SP02-02", "B021", "02_PDF_Raw/B/B-E-01_2023_Anaclerio_Numerical_Underexpanded_HydrogenJets_ICE.pdf", 11, "Figs. 18-19", "combined", (0.02, 0.04, 0.55, 0.98), "Steady/unsteady Mach-disk and scalar-field comparison", "Hydrogen; one-jet NPR 12.7 case", "S06", "FIG-02", "Left-column crop retaining axes and legends", "LOC-B021-0006"),
    PanelSpec("SP02-03", "B029", "02_PDF_Raw/B/B-E-09_2026_Sang_InjectorPressureBuildUp_Underexpanded_HydrogenJet.pdf", 8, "Figs. 9-11", "combined", (0.02, 0.04, 0.58, 0.98), "Injector pressure build-up and jet-field response", "Hydrogen injector; transient pressure histories", "S06", "FIG-02", "Left-column crop retaining axes, legends, and contour scale", "LOC-B029-0008"),
    PanelSpec("SP04-01", "C012", "02_PDF_Raw/C/C-SD-05_2019_BiasioriPoulanges_HighMagnification_Shadowgraphy_DropBreakup.pdf", 3, "Fig. 3", "a-n", (0.00, 0.02, 0.58, 0.98), "High-magnification sheet, rim, ligament, and fragment evolution", "Water; We 1350", "S10", "FIG-04", "Figure crop retaining scale bar and time labels", "PDF p.3, Fig. 3"),
    PanelSpec("SP04-02", "C013", "02_PDF_Raw/C/C-SD-06_2020_Dorschner_LigamentFormation_Shedding_DropletAerobreakup.pdf", 22, "Fig. 15", "sequence", (0.02, 0.03, 0.57, 0.94), "Recurrent ligament formation and shedding", "Water; We 295", "S10", "FIG-04", "Left-column figure crop retaining annotations", "PDF p.22, Fig. 15"),
    PanelSpec("SP04-03", "C016", "02_PDF_Raw/C/C-SD-09_2021_Sharma_ShockInduced_DropletAerobreakup.pdf", 13, "Fig. 6", "a-d", (0.46, 0.03, 0.99, 0.43), "SIE and RTP breakup morphologies", "Water; M_s 1.12-1.45; case-specific We and Re", "S10", "FIG-04", "Upper-right crop retaining scale bars and labels", "LOC-C016-0004"),
    PanelSpec("SP04-04", "D018", "02_PDF_Raw/D/D-DW-03_2026_Yang_RP3_Droplet_DeformationBreakup_Detonation.pdf", 10, "Fig. 15", "sequence", (0.02, 0.03, 0.96, 0.27), "KHI-dominant deformation, coupled KHI-RTI, and breakup", "RP-3; d0 530 um; detonation Ma 7.03", "S10", "FIG-04", "Top-strip crop retaining stage and time annotations", "LOC-D018-0008"),
    PanelSpec("SP05-01", "C035", "02_PDF_Raw/C/C-MD-03_2021_Wang_ShockInduced_TandemDroplet_Breakup.pdf", 5, "Fig. 5", "sequence", (0.42, 0.33, 0.98, 0.89), "Tandem-droplet wake shielding and differential deformation", "Water; tandem pair; S/D and We case-specific", "S14", "FIG-05", "Right-column figure crop retaining dimensional labels", "LOC-C035-0004"),
    PanelSpec("SP05-02", "C036", "02_PDF_Raw/C/C-MD-04_2026_Guo_ShockInduced_ParallelDroplets_Aerobreakup.pdf", 5, "Fig. 2", "sequence", (0.03, 0.04, 0.97, 0.77), "Parallel-droplet channel opening and closure", "Parallel droplets; We 16.7; multiple L/D", "S14", "FIG-05", "Figure crop retaining spacing labels and time sequence", "LOC-C036-0003"),
    PanelSpec("SP05-03", "C033", "02_PDF_Raw/C/C-MD-01_2011_Chauvin_PlanarShock_TwoPhase_GasLiquid_Medium.pdf", 5, "Fig. 2", "sequence", (0.54, 0.50, 0.99, 0.99), "Shock propagation through a droplet cloud", "Water-droplet cloud; planar shock", "S14", "FIG-05", "Lower-right crop retaining station layout and pressure traces", "PDF p.5, Fig. 2"),
    PanelSpec("SP06-01", "C024", "02_PDF_Raw/C/C-ST-04_2023_DukeWalker_SmallDroplet_BreakupEvaporation_HighWeber.pdf", 7, "Fig. 7", "a-d", (0.03, 0.03, 0.96, 0.43), "Fragment-cloud formation and optical disappearance", "Acetone droplets; M about 2.09; high We", "S18", "FIG-06", "Top figure crop retaining time labels and flow direction", "LOC-C024-0005"),
    PanelSpec("SP06-02", "C031", "02_PDF_Raw/C/C-PC-05_2025_Song_3D_PhaseChange_ShockDroplet_Interaction.pdf", 11, "Fig. 7", "a-c", (0.11, 0.05, 0.96, 0.59), "No-phase-change, evaporation, and condensation morphologies", "n-dodecane; modeled Mach 1.47 family", "S18", "FIG-06", "Upper figure crop retaining branch labels and times", "LOC-C031-0008"),
    PanelSpec("SP06-03", "C032", "02_PDF_Raw/C/C-PC-06_2016_Strotos_NDecane_DropletBreakup_HighTemperatureGas.pdf", 5, "Figs. 3-4", "combined", (0.08, 0.03, 0.96, 0.80), "Heating-dependent deformation and breakup", "n-decane; 800 and 1000 K; We 15-90", "S18", "FIG-06", "Figure crop retaining We/Re labels and time contours", "PDF p.5, Figs. 3-4"),
    PanelSpec("SP06-04", "C026", "02_PDF_Raw/C/C-ST-06_2025_DukeWalker_SmallDroplet_DeformationAcceleration_HighSpeed.pdf", 6, "Fig. 2", "A-E", (0.46, 0.06, 0.99, 0.76), "Size- and We-dependent small-droplet evolution", "Acetone; d0 about 55-200 um; case-specific Mach/We", "S18", "FIG-06", "Right-side crop retaining row labels and image times", "PDF p.6, Fig. 2"),
    PanelSpec("SP07-01", "A022", "02_PDF_Raw/A/A-H2-05_2024_Rorimpandey_JetInteractionAngle_H2DDI.pdf", 3, "Fig. 2", "a-c", (0.46, 0.03, 0.99, 0.72), "Hydrogen-pilot jet intersection geometry", "H2/diesel DDI; 12, 15, and 19 deg arrangements", "S22", "FIG-07", "Right-column crop retaining injector geometry dimensions", "LOC-A022-0006"),
    PanelSpec("SP07-02", "A026", "02_PDF_Raw/A/A-H2-09_2025_Heaton_HydrogenInjectionTiming_EnergyProportion_Flame.pdf", 10, "Fig. 10", "sequence", (0.03, 0.03, 0.97, 0.65), "Timing-conditioned diesel-pilot and early hydrogen flame development", "H2/diesel optical engine; timing sweep", "S22", "FIG-07", "Figure crop retaining crank-angle and timing labels", "LOC-A026-0005"),
    PanelSpec("SP07-03", "A007", "02_PDF_Raw/A/A-NG-07_2018_Fink_DieselPilot_NGJet_SpatialTemporalInteraction.pdf", 6, "Fig. 10", "map", (0.02, 0.50, 0.52, 0.99), "Spatial-temporal natural-gas ignition-delay map", "Diesel pilot/natural gas; RCEM", "S22", "FIG-07", "Lower-left crop retaining axes and color scale", "LOC-A007-0005"),
    PanelSpec("SP08-01", "D018", "02_PDF_Raw/D/D-DW-03_2026_Yang_RP3_Droplet_DeformationBreakup_Detonation.pdf", 5, "Fig. 5", "a-f", (0.02, 0.18, 0.98, 0.78), "Water/RP-3 detonation-driven breakup comparison", "Detonation Ma about 6-7; water and RP-3", "S26", "FIG-08", "Figure crop retaining flow direction, time, and case labels", "LOC-D018-0005"),
    PanelSpec("SP08-02", "D019", "02_PDF_Raw/D/D-DW-04_2026_Zou_DetonationShock_DropletInteraction_Mechanisms.pdf", 11, "Fig. 8", "a-d", (0.03, 0.03, 0.58, 0.58), "Reacting detonation versus inert-shock wave configurations", "Water; d0 4.8 mm; matched incident Mach 4.8", "S26", "FIG-08", "Upper-left crop retaining density and pressure contours", "LOC-D019-0006"),
    PanelSpec("SP08-03", "D014", "02_PDF_Raw/D/D-RDE-09_2025_Li_DropletKinematics_Distribution_Ethanol_RDE.pdf", 10, "Fig. 16", "a-b", (0.02, 0.02, 0.56, 0.44), "Wave-relative droplet position and thermal/evaporation history", "Two-phase ethanol RDE; grouped droplet histories", "S26", "FIG-08", "Upper-left crop retaining axes and position markers", "PDF p.10, Fig. 16"),
    PanelSpec("SP08-04", "D019", "02_PDF_Raw/D/D-DW-04_2026_Zou_DetonationShock_DropletInteraction_Mechanisms.pdf", 14, "Fig. 12", "a-d", (0.03, 0.03, 0.58, 0.55), "Reacting/inert post-wave pressure-field divergence", "Water; matched incident Mach 4.8", "S26", "FIG-08", "Upper-left crop retaining contour scales", "LOC-D019-0008"),
]
...
def fig01():
    c = Canvas("FIG-01 | Injector-to-ignition physical map", "Transient forcing, liquid response, transport, and open couplings")
    labels = [
    ...
    x0, y, bw, bh, gap = 35, 300, 205, 190, 14
    ...
    c.box_text(190, 670, 1420, 170, "Four unresolved couplings", "actual pilot Lagrangian load  |  canonical-to-dense-spray transfer  |  fragment-to-mixture contribution  |  mixture-to-ignition response", fill="#FFF8F0", outline=RED, dash=True)
    c.text(900, 920, "Open arrows indicate measurement needs, not established causal closure.", 24, RED, bold=True, anchor="ma")
    c.save("FIG-01")
...
def fig02():
    ...
    c.panel("SP02-01", 45, 150, 560, 650, "B009 · Fig. 8", "Mach-disk development")
    c.panel("SP02-02", 620, 150, 560, 650, "B021 · Figs. 18-19", "steady / unsteady")
    c.panel("SP02-03", 1195, 150, 560, 650, "B029 · Figs. 9-11", "pressure / jet response")
    y = 900
...
def fig03():
    ...
    c.box_text(70, 180, 490, 290, "Eulerian gas field", "p(x,t)   rho_g(x,t)   u_g(x,t)\nshocks; expansions; vortices", fill=CYAN)
    c.box_text(670, 180, 460, 290, "Droplet trajectory", "x_d(t)   u_d(t)\nposition and velocity through the evolving field", fill=PALE_ORANGE, outline=ORANGE)
    c.box_text(1240, 180, 490, 290, "Sampled loading", "p[x_d(t),t]\nrho_g[x_d(t),t]\nu_rel(t) = u_g[x_d(t),t] - u_d(t)", fill="#EEF6ED", outline=TEAL)
    c.arrow(560, 325, 670, 325)
    c.arrow(1130, 325, 1240, 325)
    c.box_text(250, 630, 390, 220, "Loading clock", "tau_load\ndirection; amplitude; duration", fill=LIGHT)
    c.box_text(705, 630, 390, 220, "Response clock", "tau_response\ndeformation; instability; breakup", fill=LIGHT)
    c.box_text(1160, 630, 390, 220, "Organizing ratio", "tau_load / tau_response\nhypothesis only", fill="#FFF8F0", outline=RED, dash=True)
    c.arrow(640, 740, 705, 740)
    c.arrow(1095, 740, 1160, 740, RED, 4, True)
    c.text(900, 960, "Actual HPDI pilot-droplet histories remain unresolved.", 30, RED, bold=True, anchor="ma")
    c.save("FIG-03")


def fig04():
    c = Canvas("FIG-04 | Compressible droplet morphology and breakup", "Direct observations reveal distinct acceleration-, shear-, and capillary-mediated structures")
    c.panel("SP04-01", 40, 150, 420, 690, "C012 · Fig. 3", "sheet / rim / mist")
    c.panel("SP04-02", 475, 150, 420, 690, "C013 · Fig. 15", "recurrent ligaments")
    c.panel("SP04-03", 910, 150, 420, 690, "C016 · Fig. 6", "SIE / RTP")
    c.panel("SP04-04", 1345, 150, 415, 690, "D018 · Fig. 15", "strong-wave analogue")
    notes = [(250, "Acceleration-driven", "RT-type piercing"), (700, "Shear-driven", "KH waves / stripping"), (1150, "Capillary response", "sheet, rim, ligaments"), (1570, "Fragment production", "daughter droplets / mist")]
    for x, title, body in notes:
        c.text(x, 895, title, 23, NAVY, bold=True, anchor="ma")
        c.text(x, 930, body, 21, DARK, anchor="ma")
    c.save("FIG-04")
...
def fig05():
    ...
    c.box_text(30, 200, 300, 600, "Canonical local response", "incident shock + post-wave flow\nno shielding\nno population feedback", fill=CYAN)
    c.panel("SP05-01", 365, 200, 300, 600, "C035 · Fig. 5", "wake shielding")
    c.panel("SP05-02", 700, 200, 300, 600, "C036 · Fig. 2", "squeeze / closure")
    c.panel("SP05-03", 1035, 200, 300, 600, "C033 · Fig. 2", "attenuation")
...
def fig06():
    ...
    for pid, x, label, cap in [
        ("SP06-01", 30, "C024 · Fig. 7", "fragment cloud"),
        ("SP06-04", 465, "C026 · Fig. 2", "size / loading"),
        ("SP06-02", 900, "C031 · Fig. 7", "phase-change branch"),
        ("SP06-03", 1335, "C032 · Figs. 3-4", "thermal-property branch"),
    ]:
        c.panel(pid, x, 145, 405, 500, label, cap)
    c.box_text(120, 730, 600, 200, "Branch A | vapor layer / blowing", "reduced interfacial shear -> suppressed KH growth", fill=CYAN, outline=BLUE)
    c.box_text(790, 730, 600, 200, "Branch B | heating / liquid properties", "reduced surface tension -> promoted deformation", fill=PALE_ORANGE, outline=ORANGE)
    c.box_text(1450, 715, 310, 235, "Conditional downstream state", "fragment distribution -> transport -> evaporation -> vapor redistribution", fill="#FFF8F0", outline=RED, dash=True, title_size=19, body_size=18)
    c.arrow(720, 830, 790, 830, GRAY, 3, True)
    c.arrow(1390, 830, 1460, 830, RED, 4, True)
    c.text(900, 990, "Breakup does not imply faster evaporation or improved mixing without the intervening state and residence scales.", 23, RED, bold=True, anchor="ma")
    c.save("FIG-06")
...
def fig07():
    ...
    c.panel("SP07-01", 40, 385, 540, 510, "A022 · Fig. 2", "jet geometry")
    c.panel("SP07-02", 630, 385, 540, 510, "A026 · Fig. 10", "timing / flame")
    c.panel("SP07-03", 1220, 385, 540, 510, "A007 · Fig. 10", "ignition-delay map")
...
def fig08():
    ...
    c.panel("SP08-01", 35, 145, 540, 390, "D018 · Fig. 5", "liquid-dependent breakup")
    c.panel("SP08-02", 600, 145, 540, 390, "D019 · Fig. 8", "reacting / inert waves")
    c.panel("SP08-03", 35, 560, 540, 390, "D014 · Fig. 16", "thermal history")
    c.panel("SP08-04", 600, 560, 540, 390, "D019 · Fig. 12", "post-wave divergence")
...
def fig09():
    c = Canvas("FIG-09 | Competing clocks and unresolved transported quantities", "Cross-scale organization without assuming a universal timescale ratio")
    clocks = ["tau_inject", "tau_wave", "tau_load", "tau_response", "tau_breakup", "tau_evap", "tau_mix", "tau_ign"]
    x0, y, bw, gap = 55, 220, 190, 26
    for i, label in enumerate(clocks):
        x = x0 + i * (bw + gap)
        c.box_text(x, y, bw, 150, label, ["injector", "wave", "droplet load", "liquid response", "fragmentation", "phase change", "transport", "chemistry"][i], fill=CYAN if i < 3 else (PALE_ORANGE if i < 6 else LIGHT))
        if i < len(clocks) - 1:
            c.arrow(x + bw, y + 75, x + bw + gap - 4, y + 75, GRAY, 3)
    c.text(900, 470, "Transported variables", 28, NAVY, bold=True, anchor="ma")
    vars_ = ["pressure / density history", "relative velocity", "fragment size / velocity", "temperature / vapor", "mixture / radicals"]
    for i, label in enumerate(vars_):
        c.rect(90 + i * 340, 520, 300, 100, fill=LIGHT, outline=BLUE, width=2, radius=12)
        c.text(240 + i * 340, 550, label, 21, DARK, anchor="ma")
    c.box_text(90, 750, 360, 190, "Unresolved quantity 1", "actual pilot-droplet load history", fill="#FFF8F0", outline=RED, dash=True)
    c.box_text(510, 750, 360, 190, "Unresolved quantity 2", "dense-spray collective response", fill="#FFF8F0", outline=RED, dash=True)
    c.box_text(930, 750, 360, 190, "Unresolved quantity 3", "fragment-to-mixture contribution", fill="#FFF8F0", outline=RED, dash=True)
    c.box_text(1350, 750, 360, 190, "Unresolved quantity 4", "mixture-to-ignition response", fill="#FFF8F0", outline=RED, dash=True)
    c.save("FIG-09")
...
print(f"Generated Phase 14R figures and panel registry under {OUT}")
